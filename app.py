...
# ---------------------------------------------------------------------------
def lazy_import():
    global torch, torchaudio, sf, nr, textwrap   #global module space
...
# TTSApp
# ===========================================================================
class TTSApp(tk.Tk): #inherit form tkinter
    def __init__(self):
        super().__init__()
    ...
    def _build_ui(self):
        self.rowconfigure(0, weight=1) #size of 0(first) row 
        self.columnconfigure(0, weight=1)

        # ── Scrollable canvas wrapping all content ─────────────────────────
       
        canvas  = tk.Canvas(self, bg=DARK_BG, highlightthickness=0)
        # vertical scrollbar linked to canvas y-axis
        vscroll = ttk.Scrollbar(self, orient="vertical", command=canvas.yview)
        # use scrolling in canvas
        canvas.configure(yscrollcommand=vscroll.set)
        # scrollbar on the right edge, stretches full height
        #n  = up, s  = down, e  = right, w  = left
        vscroll.grid(row=0, column=1, sticky="ns") # position inside parent
        # canvas fills the entire remaining space
        canvas.grid(row=0, column=0, sticky="nsew")

        # inner frame holds all actual content — canvas only displays it
        inner  = tk.Frame(canvas, bg=DARK_BG)
        # place inner frame inside canvas at top-left corner
        win_id = canvas.create_window((0, 0), window=inner, anchor="nw")

        # when inner frame resizes, update the scrollable area to match its new size
        inner.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all"))) #set canvas rectangle size as scrollable
        # when window resizes, stretch inner frame to match canvas width
        canvas.bind("<Configure>", lambda e: canvas.itemconfig(win_id, width=e.width))
        # mousewheel scrolling — delta/120 gives scroll units, negated to match scroll direction
        canvas.bind_all("<MouseWheel>", lambda e: canvas.yview_scroll(int(-1 * (e.delta / 120)), "units")) #bind_all - listen ewerywhere

        # all rows fixed size — content determines height, no stretching
        inner.rowconfigure(0, weight=0)
        inner.rowconfigure(1, weight=0)
        inner.rowconfigure(2, weight=0)
        inner.rowconfigure(3, weight=0)
        # single column stretches to fill full width
        inner.columnconfigure(0, weight=1)

        # ── Row 0: header ──────────────────────────────────────────────────
        hdr = tk.Frame(inner, bg=DARK_BG)
        hdr.grid(row=0, column=0, sticky="ew", padx=28, pady=(22, 0))
        tk.Label(hdr, text="SpeechT5",   font=FONT_TITLE, fg=ACCENT,    bg=DARK_BG).pack(side="left") #pack - anhor view (invisible witchout)
    ...
    def _build_left(self, parent):
        ...
        self._model_var = tk.StringVar(value=list(PRESET_MODELS.keys())[0]) #stringvar reacts on changes (reactive) & update ui
        ...
        self._path_entry("Checkpoint (folder):", "checkpoint_path", #checkpoint_path - value name
    ...
    def _path_entry(self, label, key, parent, is_dir=False, save=False):
        ...
        tk.Entry(fr, textvariable=var, font=FONT_MONO, bg=PANEL_BG, fg=TEXT_MAIN, #one line input
                 insertbackground=ACCENT, relief="flat", bd=4,
                 highlightthickness=1, highlightbackground=BORDER,
    ...
    def _on_model_select(self, _=None): #not use event
        self._toggle_custom_frame()

    def _toggle_custom_frame(self): #show or hide selection ui
